/*****************************
* Revision el 29-01-2015 Error actualizando existencia
 * ***************************/
using System;
...
{
    public partial class frmMovInventario : frmBase
    {
            ...
            try
            {
                ...
                foreach (DataGridViewRow registro in grdDetalle.Rows)
                {
                    if (Convert.ToString(registro.Cells["cboCodigo"].Value) != "") // Si no esta en blanco el codigo de la cuenta
                    {
                        ...
                        {
                            MessageBox.Show("La Cantidad no pueden ser cero, favor verificar", "Sistema SysHospitalARD v1.0", MessageBoxButtons.OK,
                            MessageBoxIcon.Information);
                            oTransaccion.Rollback();
                            oCnn.Close();
                            this.CursorDefault();
                        ...
                        sbQuery.Append(" where trim(mproduct.cod_pro) = '" + cCod_Pro + "'");
