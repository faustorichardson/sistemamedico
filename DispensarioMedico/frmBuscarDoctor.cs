...
{
    public partial class frmBuscarDoctor : frmBase
    {
        ...
        private void frmBuscarDoctor_Load(object sender, EventArgs e)
        {
        ...
        private void txtBuscar_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                if (this.txtBuscar.Text != "")
                {
                    ...
                    DataTable dsCatalogo = clsProcesos.DatosGeneral("vdoctores", " where upper(doctores_nombre)  like " + cBuscar + " or upper(doctores_apellido) like  " + cBuscar, " order by rangoabrev,doctores_apellido ");
                    ...
                    if (dsCatalogo.Rows.Count > 0)
                    {
                        ...
                        {
                            this.grdCatalogo.Rows.Add(registro["doctores_cedula"], registro["doctores_nombre"].ToString().Trim() + " " + registro["doctores_apellido"].ToString().Trim(), registro["rangoabrev"]);
                        }
                    }
                    else
                    {
